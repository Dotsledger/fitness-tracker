...
function buildChrome() {
  // Barra inferior de navegación (móvil-first).
...
function configBanner() {
  ...
  return el("div", { class: "config-banner" }, [
    ...
    ". Hasta entonces no se cargarán datos.",
  ]);
}
...
function boot() {
  ...
  // Si Supabase no está configurado, ninguna vista puede leer datos: en vez de
  // dejar que cada fetch falle, mostramos un panel claro.
  const guard = (fn) => (root, param) => {
    if (!CONFIGURED) {
      root.innerHTML =
        '<div class="empty"><div class="empty__title">Configura Supabase para empezar</div>' +
        '<div class="empty__sub">Edita <code>js/config.js</code> con tu URL y anon key, ' +
        'y ejecuta <code>db/schema.sql</code> en Supabase.</div></div>';
      return;
    }
...
// Registrar el service worker (PWA). Ruta relativa para funcionar en el
